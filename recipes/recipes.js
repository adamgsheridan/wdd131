...
function ratingTemplate(rating) {
    ...
    for (let i = 1; i <= 5; i++) {
        if (i <= rating) {
            totalStars += `<span aria-hidden="true" class="icon-star">⭐</span>`
        } else if (i <= rating + 0.5 && i - 0.5 === rating) {
            totalStars += `<span aria-hidden="true" class="icon-star-half">⯪</span>`
        } else {
            totalStars += `<span aria-hidden="true" class="icon-star-empty">☆</span>`
        }
    }
...
function recipeTemplate(recipe) {
    return `<div class="recipe-content">
